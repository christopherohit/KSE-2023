...
class ResNet_V101(object):
    ...
    def build_network(self):
        ...
        self.outputs = self.build_encoder(self.encoding)
    ...
    def build_encoder(self):
        ...
        outputs = self._bottleneck_resblock(outputs, 512, '3a', haft_size = True, identity_connection = False)
    ...
    def _bottleneck_resblock(self, x, num_o, name, haft_size = False, identity_connection = True):
        first_s = 2 if haft_size else 1
        ...
        o_b2a = self._batch_norm(o_b2a, name = 'bn%s_branch2a' % name, is_train = False, activation_fn = tf.nn.relu)
        
        o_b2b = self._conv2d(o_b2a, 3, num_o/ 4, 1, name = 'res%s_branch2b' % name)
        o_b2b = self._batch_norm(o_b2b, name = 'bn%s_branch2b' % name, is_train = False, activation_fn = tf.nn.relu)
        
        o_b2c = self._conv2d(o_b2b, 1, num_o, 1, name = 'res%s_branch2c' % name)
        o_b2c = self._batch_norm(o_b2c, name = 'bn%s_branch2c' % name, is_train = False, activation_fn = None)
    ...
    def _ASPP(self, x, num_o, dilations):
        ...
        for i, d in enumerate(dilations):
            o.append(self._dilated_conv2d(x, 3, num_o, d, name= 'fc1_voc12_c%d', biased= True))
        return self._add(o, name= 'fc_voc12')
    ...
    def _conv2d(self, x, kernel_size, num_o, stride, name, biased= False):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            w = tf.compat.v1.get_variable('weights', shape=[kernel_size, kernel_size, num_x, num_o])
            s = [1, stride, stride, 1]
            o = tf.nn.conv2d(input=x, filters=w, strides=s, padding='SAME')
            if biased:
                b = tf.compat.v1.get_variable('biases', shape=[num_o])
    ...
    def _max_pool2d(self, x, kernel_size, stride, name):
        ...
        s = [1, stride, stride, 1]
        return tf.nn.max_pool2d(input=x, ksize=k, strides=s, padding= 'SAME', name= name)
...
class ResNet_segmentation(object):
    ...
    def build_encoder(self):
        ...
        with tf.compat.v1.variable_scope(scope_name) as scope:
            ...
            with tf.compat.v1.variable_scope('block3') as scope:
                ...
                print("after block3:", outputs.shape)
            with tf.compat.v1.variable_scope('block4') as scope:
    ...
    def _batch_norm(self, x, name, is_training, activation_fn, trainable=False):
        ...
        with tf.compat.v1.variable_scope(name+'/BatchNorm') as scope:
            o = tf.contrib.layers.batch_norm(
                x,
                scale=True,
                activation_fn=activation_fn,
