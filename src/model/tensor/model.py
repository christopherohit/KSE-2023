...
class Model(object):
	...
	def train_setup(self):
		...
		# Output size
		output_shape = tf.shape(input=raw_output)
		...
		# Loss function
		self.reduced_loss = tf.reduce_mean(input_tensor=loss) + tf.add_n(l2_losses)
		...
		# Image summary.
		images_summary = tf.compat.v1.py_func(inv_preprocess, [self.image_batch, 2, IMG_MEAN], tf.uint8)
		labels_summary = tf.compat.v1.py_func(decode_labels, [self.label_batch, 2, self.conf.num_classes], tf.uint8)
		preds_summary = tf.compat.v1.py_func(decode_labels, [self.pred, 2, self.conf.num_classes], tf.uint8)
