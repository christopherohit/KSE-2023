...
from network import *
from model.tensor.utils import ImageReader, decode_labels, inv_preprocess, prepare_label, write_log, read_labeled_image_list
...
class Model_msc(object):
	...
	def train(self):
		...
		# Start queue threads.
		threads = tf.compat.v1.train.start_queue_runners(coord=self.coord, sess=self.sess)
	...
	def train_setup(self):
		tf.compat.v1.set_random_seed(self.conf.random_seed)
		...
		with tf.compat.v1.name_scope("create_inputs"):
			...
			image_batch_075 = tf.image.resize(self.image_batch, [int(h * 0.75), int(w * 0.75)])
			image_batch_05 = tf.image.resize(self.image_batch, [int(h * 0.5), int(w * 0.5)])
		...
		if self.conf.encoder_name not in ['res101', 'res50', 'deeplab']:
			...
			with tf.compat.v1.variable_scope('', reuse=False):
				net = ResNet_V101(self.image_batch, self.conf.num_classes, True)
			with tf.compat.v1.variable_scope('', reuse=True):
				net075 = ResNet_V101(image_batch_075, self.conf.num_classes, True)
			with tf.compat.v1.variable_scope('', reuse=True):
				net05 = ResNet_V101(image_batch_05, self.conf.num_classes, True)
			# Variables that load from pre-trained model.
			restore_var = [v for v in tf.compat.v1.global_variables() if 'fc' not in v.name]
			# Trainable Variables
			all_trainable = tf.compat.v1.trainable_variables()
			# Fine-tune part
			...
			# Variables that load from pre-trained model.
			restore_var = [v for v in tf.compat.v1.global_variables() if 'resnet_v1' in v.name]
			# Trainable Variables
			all_trainable = tf.compat.v1.trainable_variables()
			# Fine-tune part
		...
		raw_output = tf.reduce_max(input_tensor=tf.stack([raw_output100,
									tf.image.resize(raw_output075, tf.shape(input=raw_output100)[1:3,]),
									tf.image.resize(raw_output05, tf.shape(input=raw_output100)[1:3,])]), axis=0)
		...
		indices = tf.squeeze(tf.compat.v1.where(tf.less_equal(raw_gt, self.conf.num_classes - 1)), 1)
		indices075 = tf.squeeze(tf.compat.v1.where(tf.less_equal(raw_gt075, self.conf.num_classes - 1)), 1)
		indices05 = tf.squeeze(tf.compat.v1.where(tf.less_equal(raw_gt05, self.conf.num_classes - 1)), 1)
		...
		# Loss function
		self.reduced_loss = tf.reduce_mean(input_tensor=loss) + tf.reduce_mean(input_tensor=loss100) + tf.reduce_mean(input_tensor=loss075) + tf.reduce_mean(input_tensor=loss05) + tf.add_n(l2_losses)

		# Define optimizers
		# 'poly' learning rate
		base_lr = tf.constant(self.conf.learning_rate)
		self.curr_step = tf.compat.v1.placeholder(dtype=tf.float32, shape=())
		...
		# layer.
		opt_encoder = tf.compat.v1.train.MomentumOptimizer(learning_rate, self.conf.momentum)
		opt_decoder_w = tf.compat.v1.train.MomentumOptimizer(learning_rate * 10.0, self.conf.momentum)
		opt_decoder_b = tf.compat.v1.train.MomentumOptimizer(learning_rate * 20.0, self.conf.momentum)
		...
		# Compute grads
		grads = tf.gradients(ys=self.reduced_loss, xs=encoder_trainable + decoder_w_trainable + decoder_b_trainable)
		...
		update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS) # for collecting moving_mean and moving_variance
		with tf.control_dependencies(update_ops):
			self.train_op = tf.group(train_op_conv, train_op_fc_w, train_op_fc_b)

		# Saver for storing checkpoints of the model
		self.saver = tf.compat.v1.train.Saver(var_list=tf.compat.v1.global_variables(), max_to_keep=0)

		# Loader for loading the pre-trained model
		self.loader = tf.compat.v1.train.Saver(var_list=restore_var)

		# Training summary
		# Processed predictions: for visualisation.
		raw_output_up = tf.image.resize(raw_output, input_size, method=tf.image.ResizeMethod.BILINEAR)
		raw_output_up = tf.argmax(input=raw_output_up, axis=3)
		self.pred = tf.expand_dims(raw_output_up, axis=3)
		# Image summary.
		images_summary = tf.compat.v1.py_func(inv_preprocess, [self.image_batch, 1, IMG_MEAN], tf.uint8)
		labels_summary = tf.compat.v1.py_func(decode_labels, [self.label_batch, 1, self.conf.num_classes], tf.uint8)
		preds_summary = tf.compat.v1.py_func(decode_labels, [self.pred, 1, self.conf.num_classes], tf.uint8)
		self.total_summary = tf.compat.v1.summary.image('images',
			tf.concat(axis=2, values=[images_summary, labels_summary, preds_summary]),
			max_outputs=1) # Concatenate row-wise.
		if not os.path.exists(self.conf.logdir):
			os.makedirs(self.conf.logdir)
		self.summary_writer = tf.compat.v1.summary.FileWriter(self.conf.logdir, graph=tf.compat.v1.get_default_graph())
	...
	def test_setup(self):
		...
		self.image_batch, self.label_batch = tf.expand_dims(image, axis=0), tf.expand_dims(label, axis=0)
		h_orig, w_orig = tf.cast(tf.shape(input=self.image_batch)[1], dtype=tf.float32), tf.cast(tf.shape(input=self.image_batch)[2], dtype=tf.float32)
		image_batch_075 = tf.image.resize(self.image_batch, tf.stack([tf.cast(tf.multiply(h_orig, 0.75), dtype=tf.int32), tf.cast(tf.multiply(w_orig, 0.75), dtype=tf.int32)]))
		image_batch_05 = tf.image.resize(self.image_batch, tf.stack([tf.cast(tf.multiply(h_orig, 0.5), dtype=tf.int32), tf.cast(tf.multiply(w_orig, 0.5), dtype=tf.int32)]))
		...
		if self.conf.encoder_name not in ['res101', 'res50', 'deeplab']:
			...
			with tf.compat.v1.variable_scope('', reuse=False):
				net = ResNet_V101(self.image_batch, self.conf.num_classes, False)
			with tf.compat.v1.variable_scope('', reuse=True):
				net075 = ResNet_V101(image_batch_075, self.conf.num_classes, False)
			with tf.compat.v1.variable_scope('', reuse=True):
				net05 = ResNet_V101(image_batch_05, self.conf.num_classes, False)
		else:
			with tf.compat.v1.variable_scope('', reuse=False):
		...
		raw_output = tf.reduce_max(input_tensor=tf.stack([raw_output100,
									tf.image.resize(raw_output075, tf.shape(input=raw_output100)[1:3,]),
									tf.image.resize(raw_output05, tf.shape(input=raw_output100)[1:3,])]), axis=0)
		raw_output = tf.image.resize(raw_output, tf.shape(input=self.image_batch)[1:3,], method=tf.image.ResizeMethod.BILINEAR)
		...
		# fix for tf 1.3.0
		gt = tf.compat.v1.where(temp, gt, tf.cast(temp, tf.uint8))
		...
		self.loader = tf.compat.v1.train.Saver(var_list=tf.compat.v1.global_variables())
	...
	def predict_setup(self):
		...
		with tf.compat.v1.name_scope("create_inputs"):
			...
			image, label = reader.image, reader.label # [h, w, 3 or 1]
		# Add one batch dimension [1, h, w, 3 or 1]
		image_batch, label_batch = tf.expand_dims(image, axis=0), tf.expand_dims(label, axis=0)
		h_orig, w_orig = tf.cast(tf.shape(input=image_batch)[1], dtype=tf.float32), tf.cast(tf.shape(input=image_batch)[2], dtype=tf.float32)
		image_batch_075 = tf.image.resize(image_batch, tf.stack([tf.cast(tf.multiply(h_orig, 0.75), dtype=tf.int32), tf.cast(tf.multiply(w_orig, 0.75), dtype=tf.int32)]))
		image_batch_05 = tf.image.resize(image_batch, tf.stack([tf.cast(tf.multiply(h_orig, 0.5), dtype=tf.int32), tf.cast(tf.multiply(w_orig, 0.5), dtype=tf.int32)]))
		...
		if self.conf.encoder_name not in ['res101', 'res50', 'deeplab']:
			...
			with tf.compat.v1.variable_scope('', reuse=False):
				net = ResNet_V101(image_batch, self.conf.num_classes, False)
			with tf.compat.v1.variable_scope('', reuse=True):
				net075 = ResNet_V101(image_batch_075, self.conf.num_classes, False)
			with tf.compat.v1.variable_scope('', reuse=True):
				net05 = ResNet_V101(image_batch_05, self.conf.num_classes, False)
		...
		raw_output = tf.reduce_max(input_tensor=tf.stack([raw_output100,
									tf.image.resize(raw_output075, tf.shape(input=raw_output100)[1:3,]),
									tf.image.resize(raw_output05, tf.shape(input=raw_output100)[1:3,])]), axis=0)
		raw_output = tf.image.resize(raw_output, tf.shape(input=image_batch)[1:3,], method=tf.image.ResizeMethod.BILINEAR)
		raw_output = tf.argmax(input=raw_output, axis=3)
		self.pred = tf.cast(tf.expand_dims(raw_output, axis=3), tf.uint8)
